...
col2skip = zeros(sz, 4-number_of_sensors);
current=zeros(sz,1);
realpos=zeros(sz,1);
sentpos=zeros(sz,1);
...
set(gca,'xticklabel',(tickslabels))
ylabel('q')
title('Closure position in step experiment')
subplot(2,1,2)
plot(sumofFSR)
grid on

title('Force from FSRs in step experiment')
...
scatter(q,sumofFSRcutted); title(sprintf('cutted transient: %d.0 second',skip_first/100)); xlabel('qr');ylabel('force (N)')
axis([0 19000 0 9])
grid on
%% figure 7
% q vs force without any transient cutting
figure
raw_q=data(:,idx_sentpos);
raw_force=sum(data(:,1:number_of_sensors),2);

scatter(raw_q,raw_force);title('q vs. force'); xlabel('qr'); ylabel('force (N)')
axis([0 19000 0 9])
grid on
