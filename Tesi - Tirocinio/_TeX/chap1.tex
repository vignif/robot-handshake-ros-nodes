...
Where $F_r$ is the force during the handshake applied by the robot on the participant's hand.
...
In case of emergency the file will be saved and the last data can be used to understand the configuration just before the emergency trigger.\\
...
\newpage
\section{Step input}
The simplest signal that can be sent to the Pisa/IIT SoftHand is a step signal on the reference position, in this way the participant's response can be evaluated. The experiments lasts 6 seconds in total, the parameter are selected as follows: $t_0 = 3s$, $k_0 = 8000 $, $k_1 = 15000$.
